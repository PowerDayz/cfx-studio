...
function diag(severity: TrustDiagnosticSeverity, message: string): TrustDiagnostic {
...
describe('diagOverlay', () => {
	...
	it('returns diag-info for a single info-severity diagnostic', () => {
		const r = diagOverlay([diag(TrustDiagnosticSeverity.Info, 'fyi')]);
		expect(r.className).toBe('diag-info');
		expect(r.title).toBe('[info] fyi');
	});

	it('returns diag-warning for a single warning', () => {
		const r = diagOverlay([diag(TrustDiagnosticSeverity.Warning, 'careful')]);
	...
	it('returns diag-error and joins both messages when warning + error are present (error wins on severity priority)', () => {
		const ds = [
			diag(TrustDiagnosticSeverity.Warning, 'first'),
			diag(TrustDiagnosticSeverity.Error, 'second'),
		];
	...
	it('prefers error over info', () => {
		const ds = [
			diag(TrustDiagnosticSeverity.Info, 'a'),
			diag(TrustDiagnosticSeverity.Error, 'b'),
			diag(TrustDiagnosticSeverity.Info, 'c'),
		];
	...
	it('prefers warning over info when no error is present', () => {
		const ds = [
			diag(TrustDiagnosticSeverity.Info, 'a'),
			diag(TrustDiagnosticSeverity.Warning, 'b'),
		];
