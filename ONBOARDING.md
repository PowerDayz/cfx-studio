...
The "everything in source" architecture (§1) is in place. The Cfx
contribution at `src/vs/workbench/contrib/cfx/` is fully built out —
~120 files across `common/`, `browser/`, `electron-sandbox/`, `node/`,
and `_shared/` — and compiles as part of the normal fork build.

Implemented subsystems (see the shape in §2):

- **Game-mode detection** — FiveM vs RedM from `server.cfg` /
  `fxmanifest.lua`.
- **Resources view** — replaces the Explorer; a tree of `fxmanifest.lua`
  folders with running / stopped / errored badges from log parsing.
- **FXServer manager** — Play / Stop / Restart, external process
  control, on-demand artifact download (FiveM and RedM builds).
- **Console** — FXServer output routed into a panel, ANSI-aware.
- **`.fxgraph` visual editor** — a React-Flow webview that compiles to
  Lua on save, with a radial quick-add menu and node catalog.
- **Natives reference** — a searchable tree over the FiveM/RedM
  catalogues; both `natives-fivem.json` and `natives-redm.json` are
  vendored in-tree.
- **Lua support** — generates `.luarc.json` + native typings per
  workspace and drives the Lua language server.
- **Scaffolds** — New Resource templates (Lua / TypeScript / visual).
- **MCP server** — `cfx-mcp/` exposes the running IDE to AI clients
  (see `MCP_SETUP.md`).

Runtime QA status is not tracked in this document — use the issue
tracker and hands-on testing as the source of truth for what is
verified end-to-end.
...
- Node.js (system-installed) to bootstrap; the dev build then fetches a
  portable Node into `.toolchain/` at the repo root for everything else.
- On Windows: Visual Studio 2022 with the C++ workload, plus the
  "MSVC v143 — C++ x64/x86 Spectre-mitigated libs (Latest)" component.
  The build script auto-detects the install via `vswhere.exe`.
- Python 3.x.
...
The Vite-built `.fxgraph` webview bundle and the webview's transient
`node_modules` are gitignored in the fork — they are regenerated by
`npm run cfx:dev` from the tracked sources under
`src/vs/workbench/contrib/cfx/` (the `_shared/` libs and the webview's
`src/`).
